...
`;
    ...
    function initTaskList() {
        ...
        const savedPosition = GM_getValue('controllerPosition', null);
        const defaultPosition = { x: window.innerWidth - 68, y: window.innerHeight * 0.1 };
        let buttonPosition = savedPosition ? JSON.parse(savedPosition) : defaultPosition;
        ...
        const isOnLeftSide = () => buttonPosition.x < window.innerWidth / 2;
        if (!controller) {
        ...
        const updateContainerPosition = () => {
            if (!container) return;
            const onLeft = isOnLeftSide();
            if (onLeft) {
                container.style.left = (buttonPosition.x + 60) + 'px';
                container.style.right = 'auto';
            } else {
                container.style.right = (window.innerWidth - buttonPosition.x + 12) + 'px';
                container.style.left = 'auto';
            }
            container.style.top = Math.max(20, Math.min(buttonPosition.y, window.innerHeight - 400)) + 'px';
        };
        if (!container) {
            container = document.createElement('div');
            container.id = 'task-container';
            controller.title = '点击展开/收起 | 拖拽移动位置';
        ...
        const onDragStart = (e) => {
            if (e.target.closest('.controller-icon')) return;
            isDragging = true;
            hasDragged = false;
            ...
            controller.style.transition = 'transform 0.1s ease, box-shadow 0.1s ease';
            e.preventDefault();
        };
        const onDragMove = (e) => {
            ...
            controller.style.left = buttonPosition.x + 'px';
            controller.style.top = buttonPosition.y + 'px';
            if (isExpanded) {
                updateContainerPosition();
            }
        };
        const onDragEnd = () => {
            ...
            controller.style.transition = 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.3s ease';
            GM_setValue('controllerPosition', JSON.stringify(buttonPosition));
        };
        controller.addEventListener('mousedown', onDragStart);
        controller.addEventListener('touchstart', onDragStart, { passive: false });
        ...
        controller.onclick = (e) => {
            ...
            if (isExpanded) {
                ...
                controller.title = '点击收起 | 拖拽移动位置';
            } else {
                container.style.transform = 'scale(0.95)';
                container.style.opacity = '0';
                container.style.pointerEvents = 'none';
                controller.title = '点击展开 | 拖拽移动位置';
            }
        };
